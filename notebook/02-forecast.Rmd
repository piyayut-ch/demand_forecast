...
- `tsibble::as_tsibble`
- `tsbox::ts_ts`|`tsbox::ts_xts`
- `tsbox::ts_tbl`
- `tsbox::ts_wide` | `tsbox::ts_long`
...
data %>% as_tibble %>%
  summarize(
    ex_glutinous = mean(ex_glutinous)*12,
    ex_white = mean(ex_white)*12,
    ex_hommali = mean(ex_hommali)*12,
    ex_pathum = mean(ex_pathum)*12,
  )
```

```{r}
data %>% write_xlsx("rice_export_forecast.xlsx")
```

```{r}
ex_all %>% as_tibble %>% psych::describe()
```
...
ggsave('figures/acf_hommali.png', width = 5, height = 5)
```

```{r}
ggPacf(ex_hommali, lag.max = 24) +
  labs(title="") +
  theme_ipsum(
    base_size = 16,
    base_family = "Athiti Light",
...
ggsave('figures/pacf_hommali.png', width = 5, height = 5)
```

```{r}
ggAcf(ex_white, lag.max = 24) +
  labs(title="") +
  theme_ipsum(
    base_size = 16,
    base_family = "Athiti Light",
...
ggsave('figures/acf_white.png', width = 5, height = 5)
```

```{r}
ggPacf(ex_white, lag.max = 24) +
  labs(title="") +
  theme_ipsum(
    base_size = 16,
    base_family = "Athiti Light",
...
ggsave('figures/pacf_white.png', width = 5, height = 5)
```
...
```{r}
res_hommali <- forecast::ets(ex_hommali[1:(12*14),] %>% ts_ts, 
                     model = "AAA", damped = TRUE)

rmse_vec(res_hommali$fitted %>% as.numeric() %>% exp(), 
         res_hommali$x %>% as.numeric() %>% exp() )

mape_vec(res_hommali$fitted %>% as.numeric() %>% exp(), 
         res_hommali$x %>% as.numeric() %>% exp() )
```

```{r}
ets_hommali_best <- tune_ets_hommali %>%
  filter(param == 'AAA-TRUE')

ets_hommali_best['coef'][[1]][[1]] %>%
  map(function(x) {
    x[c('alpha', 'beta', 'gamma', 'phi')]
  }) %>% reduce(bind_rows) %>% map(mean)
```

```{r}
actual_1 <- res_hommali$fitted %>% exp()
pred_1 <- res_hommali$x %>% exp()
actual_2 <- ets_hommali_best$assess %>% reduce(rbind) %>% reduce(rbind) %>% exp()
pred_2 <- ets_hommali_best$pred %>% reduce(rbind) %>% reduce(rbind) %>% exp()

x <- merge(
  rbind(actual_1, actual_2), 
  rbind(pred_1, pred_2)
)

colnames(x) <- c("actual", "predict")

x <- x %>% as_tibble() %>%
  mutate(date = as.Date(zoo::index(x), frac = 1))
...
```{r}
ggplot(x, aes(x=date)) +
  geom_line( aes(y=actual/1e6, color = 'ค่าจริง'), color = "black") +
  geom_line( aes(y=predict/1e6, color = 'ค่าพยากรณ์'), color = "red", linetype = "dashed") +
  geom_vline(xintercept = as.Date("2017-01-01"), col = "navy", size=1.1) + 
  scale_color_d3() +
  expand_limits(y = 0) +
  scale_y_continuous(label = comma) +
  labs(title = 'ปริมาณส่งออกข้าวหอมมะลิ',
       x =  "ปี",
       y = 'ล้านตันข้าวสาร'
      ) +
  annotate(
    "text", x = as.Date("2010-01-01"), y = 0.01, 
    family = "Athiti", size = 5, color = "black",
    label = "in sample"
  ) +
  annotate(
    "text", x = as.Date("2019-01-01"), y = 0.01, 
    family = "Athiti", size = 5, color = "black",
    label = "ex post"
  ) +
  theme_ipsum(
...
ggsave('figures/plot_eval_ets_hommali.png', width = 10, height = 5)
```

```{r}
fcast_hommali <- forecast::ets(
  ex_hommali %>% ts_ts, model = "AAA", damped = TRUE
  ) %>% predict(h = 24) %>% as.data.frame() %>% exp()

fcast_hommali %>%
  write_xlsx("results/fcast_hommali_ets.xlsx")
```

### White

```{r}
res_white <- forecast::ets(ex_white[1:(12*14),] %>% ts_ts, 
                     model = "MAN", damped = TRUE)

rmse_vec(res_white$fitted %>% as.numeric() %>% exp(), 
         res_white$x %>% as.numeric() %>% exp() )

mape_vec(res_white$fitted %>% as.numeric() %>% exp(), 
         res_white$x %>% as.numeric() %>% exp() )
```

```{r}
ets_white_best <- tune_ets_white %>%
  filter(param == 'MAN-TRUE')

ets_white_best['coef'][[1]][[1]] %>%
  map(function(x) {
    x[c('alpha', 'beta', 'phi')]
  }) %>% reduce(bind_rows) %>% map(mean)
```
...
```{r}
actual_1 <- res_white$fitted %>% exp()
pred_1 <- res_white$x %>% exp()
actual_2 <- ets_white_best$assess %>% reduce(rbind) %>% reduce(rbind) %>% exp()
pred_2 <- ets_white_best$pred %>% reduce(rbind) %>% reduce(rbind) %>% exp()
...
ggplot(x, aes(x=date)) +
  ...
  expand_limits(y = 0) +
  scale_y_continuous(label = comma) +
  labs(title = 'ปริมาณส่งออกข้าวเจ้าขาว',
       x =  "ปี",
       y = 'ล้านตันข้าวสาร'
      ) +
  annotate(
...
ggsave('figures/plot_eval_ets_white.png', width = 10, height = 5)
```

```{r}
fcast_white %>%
  write_xlsx("results/fcast_white_ets.xlsx")fcast_white <- forecast::ets(
  ex_white %>% ts_ts, model = "MAN", damped = TRUE
  ) %>% predict(h = 24) %>% as.data.frame() %>% exp() %>%
  rownames_to_column("date") %>%
  mutate(date = as.Date(zoo::index(date), frac = 1))

fcast_white %>%
  write_xlsx("results/fcast_white_ets.xlsx")
```
...
```{r}
tune_arima_hommali %>% select(coef)
```
...
```{r}
res_hommali <- forecast::auto.arima(ex_hommali[1:(12*14),] %>% ts_ts)
...
```{r}
res_hommali <- forecast::Arima(ex_hommali[1:(12*14),] %>% ts_ts, 
                               order    = c(1, 0, 2),
                               seasonal = c(1, 0, 1)
                              )
...
ggsave('figures/plot_eval_arima_hommali.png', width = 10, height = 5)
```

### White

```{r}
res_white <- forecast::auto.arima(ex_white[1:(12*14),] %>% ts_ts)
...
```{r}
res_white <- forecast::Arima(ex_white[1:(12*14),] %>% ts_ts, 
                             order    = c(1, 0, 1),
                             seasonal = c(0, 0, 1)
                            )
...
ggsave('figures/plot_eval_arima_white.png', width = 10, height = 5)
```

## VAR

```{r}
ex_all_ <- ex_all %>% ts_tbl %>% ts_wide %>%
  mutate(d_p_h100_th = difference(p_h100_th)) %>%
  as_tsibble(index = time) %>% ts_xts
...
hommali_endo <- c('ex_hommali', 'p_h100_th')
white_endo <- c('ex_white', 'p_w5_th', 'p_w5_vn')

hommali_all   <- ex_all[, hommali_endo]
hommali_train <- ex_all[train_end , hommali_endo]
hommali_test  <- ex_all[test_start, hommali_endo]

white_all   <- ex_all[, white_endo]
white_train <- ex_all[train_end , white_endo]
white_test  <- ex_all[test_start, white_endo]
...
```{r}
VARselect(hommali_train, lag.max = 5, type = 'const')
```

```{r}
VARselect(hommali_train, lag.max = 5, type = 'const')$criteria %>% t
```

```{r}
VAR(hommali_train, p = 2, type = 'const') %>% check_VAR()
```

```{r}
vecm_test <- VECM(
  hommali_train, lag = 2, estim = "ML", 
  include = 'const', LRinclude = 'const'
)
rank.test(vecm_test) %>% summary()
```

```{r}
vecm_hommali <- VECM(
  hommali_all, lag = 2, r = 1, estim = "ML", 
  include = 'const', LRinclude = 'const'
)
```
...
```{r}
vecm_hommali %>% summary()
```

```{r}
pred_rolling_hommali <- predict_rolling(
  vecm_hommali, nroll = 12*3, n.ahead = 1,
  refit.every = 1
)
```
...
```{r}
rmse_vec(
  pred_rolling_hommali$pred[[1]] %>% exp(),
  pred_rolling_hommali$true[[1]] %>% exp()
)
...
```{r}
str(vecm_hommali_in_sample)
```

```{r}
actual <- vecm_hommali_in_sample$model[-c(1:3), 'ex_hommali'] %>% as.numeric
resid  <- vecm_hommali_in_sample$residuals[, 'ex_hommali'] %>% as.numeric
fitted <- actual - resid
```

```{r}
vecm_hommali_in_sample <- lineVar(
  hommali_train, lag = 2, r = 1, model = "VECM",
  include = 'const', LRinclude = 'const'
)
```

```{r}
vecm_hommali_in_sample <- VECM(
  hommali_train, lag = 2, r = 1, estim = "ML", 
  include = 'const', LRinclude = 'const'
)
```
...
```{r}
VARselect(white_train, lag.max = 12, type = 'const')
```

```{r}
VARselect(white_train, lag.max = 12, type = 'const')$criteria %>% t
```

```{r}
VAR(white_train, p = 7, type = 'const') %>% check_VAR()
```

```{r}
vecm_white <- lineVar(white_all, lag = 7, include = "const", model = "VAR")
```
...
```{r}
pred_rolling_white <- predict_rolling(
  vecm_white, nroll = 12*3, n.ahead = 1,
  refit.every = 1
)
```
...
```{r}
rmse_vec(
  pred_rolling_white$pred[[1]] %>% exp(),
  pred_rolling_white$true[[1]] %>% exp()
)
...
```{r}
vecm_white_in_sample <- lineVar(
  white_train, lag = 7, include = "const", model = "VAR"
)
```

```{r}
rmse_vec(
  vecm_white_in_sample$fitted.values[, 'ex_white'] %>% exp(),
  vecm_white_in_sample$model[-c(1:7), 'ex_white'] %>% as.numeric %>% exp()
)
...
```{r}
fit_hommali <- forecast::ets(
  ex_hommali %>% ts_ts, model = "AAA", damped = TRUE
)
```

```{r}
fit_hommali %>% forecast(h = 24) %>% '[['('mean') %>% exp %>% ts_tbl %>%
  write_xlsx("results/forecast_hommali_2563-2564.xlsx")
```

```{r}
fcast_hommali <- fit_hommali %>% forecast(h = 24)
fcast_hommali$x  <- fcast_hommali$x %>% exp()/1e6
fcast_hommali$mean  <- fcast_hommali$mean %>% exp/1e6
fcast_hommali$upper <- fcast_hommali$upper %>% exp/1e6
fcast_hommali$lower <- fcast_hommali$lower %>% exp/1e6
```

```{r}
fcast_hommali %>% autoplot +
  scale_color_d3() +
  expand_limits(y = 0) +
  scale_y_continuous(label = comma) +
  labs(title = 'ปริมาณส่งออกข้าวหอมมะลิ',
...
ggsave('figures/plot_forecast_hommali_final.png', width = 10, height = 5)
```

## White

```{r}
fit_white <- forecast::ets(
  ex_white %>% ts_ts, model = "MAN", damped = TRUE
)
```

```{r}
fcast_white <- fit_white %>% forecast(h = 24)
fcast_white$x  <- fcast_white$x %>% exp()/1e6
fcast_white$mean  <- fcast_white$mean %>% exp/1e6
fcast_white$upper <- fcast_white$upper %>% exp/1e6
fcast_white$lower <- fcast_white$lower %>% exp/1e6
```

```{r}
fcast_white %>% autoplot +
  scale_color_d3() +
  expand_limits(y = 0) +
  scale_y_continuous(label = comma) +
  labs(title = 'ปริมาณส่งออกข้าวเจ้าขาว',
...
ggsave('figures/plot_forecast_white_final.png', width = 10, height = 5)
```

```{r}
fit_white %>% forecast(h = 24) %>% '[['('mean') %>% exp %>% ts_tbl %>%
  write_xlsx("results/forecast_white_2563-2564.xlsx")
```
...
```{r}
x <- readRDS("results/tune_ets_white.rds")
```
